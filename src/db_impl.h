...
class TitanDBImpl : public TitanDB {
 public:
  ...
  using TitanDB::GetFPInfo;
  std::string GetFPInfo(ColumnFamilyHandle* column_family);
  ...
  using TitanDB::GCInProgress;
  bool GCInProgress(ColumnFamilyHandle* handle);
 ...
 private:
  std::map<uint32_t, FPIndex*> fp_index_map;
