...
Status TitanDBImpl::OpenImpl(const std::vector<TitanCFDescriptor>& descs,
  ...
  std::vector<FPIndex*> fp_indexs(descs.size());
  ...
  for (size_t i = 0; i < descs.size(); i++) {
	...
	fp_indexs[i] = new FPIndex(descs[i].options);
  ...
  for (size_t i = 0; i < descs.size(); i++) {
	...
	fp_index_map[(*handles)[i]->GetID()] = fp_indexs[i];
  }
...
Status TitanDBImpl::CreateColumnFamilies(
  ...
  for (size_t i = 0; i < descs.size(); i++) {
	...
	fp_indexs[i] = new FPIndex(TitanCFOptions(descs[i].options)); 
  ...
  if (s.ok()) {
    std::map<uint32_t, TitanCFOptions> column_families;
    {
      ...
      for (size_t i = 0; i < descs.size(); i++) {
		...
		fp_index_map[cf_id] = fp_indexs[i];
      }
...
Status TitanDBImpl::Put(const rocksdb::WriteOptions& options,
  ...
  auto debugger = Debugger("|db_impl.cc|TitanDBImpl.Put:");
  debugger.dprintln("key:%s, value:%s", key.data(), value.data()); 
...
Status TitanDBImpl::GetImpl(const ReadOptions& options,
                            ...
                            PinnableSlice* value) {
  Debugger debugger("|db_impl.cc|TitanDBImpl.GetImpl:");
  Status s;
  ...
  if (storage) {
	...
	debugger.dprintln("Get from index(file number: %d, offset: %d), status: %s", index.file_number, index.blob_handle.offset, s.ToString().c_str());
...
std::string TitanDBImpl::GetFPInfo(ColumnFamilyHandle* handle) {
	int cf_id = handle->GetID();
	Debugger().dprintln("GetFPInfo, cf_id: %d", cf_id);
	if(fp_index_map.find(cf_id) == fp_index_map.end()) {
		return "";
	} else {
		return fp_index_map[cf_id]->GetFPInfo();	
	}
}
