...
Status TitanDBImpl::BackgroundGC(LogBuffer* log_buffer,
  ...
  if (UNLIKELY(!blob_gc)) {
    ...
    BlobGCJob blob_gc_job(blob_gc.get(), db_, &mutex_, db_options_, env_,
                          ...
                          stats_.get(), fp_index_map[column_family_id]);
...
bool TitanDBImpl::GCInProgress(ColumnFamilyHandle* handle) {
	MutexLock l(&mutex_);
	std::unique_ptr<BlobGC> blob_gc;
	std::shared_ptr<BlobStorage> blob_storage;
	int cf_id = handle->GetID();
	if(blob_file_set_->IsColumnFamilyObsolete(cf_id)) return true;
	blob_storage = blob_file_set_->GetBlobStorage(cf_id).lock();
	if(!blob_storage) return true;
	blob_storage->ComputeGCScore();
	const auto& cf_options = blob_storage->cf_options();
	std::shared_ptr<BlobGCPicker> blob_gc_picker = 
		std::make_shared<BasicBlobGCPicker>(db_options_, cf_options, stats_.get());
	return !blob_gc_picker->GCFinished(blob_storage.get());
}
