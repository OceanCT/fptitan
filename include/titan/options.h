...
struct TitanCFOptions : public ColumnFamilyOptions {
  ...
  // If set true, Titan will use fingerprints to mark records, and later gc will
  // try to put similar records together, which can reduce blob file size a decent
  // amount. However, it also causes extra calculation when adding and moving records.
  // So it's suggested to set false if one would rather have ultimate write speed 
  // rather than space usage.
  bool fpindex_enable{true};
  int fp_index_max_size{12500};
  // int fp_feature_num{3};
  int fp_feature_num{24};
  int fp_shiftbits{8};
  int fp_super_feature_num{3};
  // int fp_super_feature_num{24};
  int fp_mask{0};
  int fp_max_match{100};  

  /*
  std::vector<uint64_t> fp_mi{110123, 1123213, 1123807}; 
  std::vector<uint64_t> fp_ai{21312311, 234321, 1000001};
  */
  
  std::vector<uint64_t> fp_mi{ 
	  1007, 459326, 48011, 935033, 831379, 530326, 384145, 
	  687288, 846569, 654457, 910678, 48431, 23678, 792419, 
	  32907, 12794, 12084, 2801, 9374, 2018, 38037, 29247, 9281, 29418 };
  std::vector<uint64_t> fp_ai{
    1623654698648, 1676314223803, 687634657152,  111346466486740, 11534664856562, 2123469903346,
    8975934662878,  148633637918,  1948934641976, 1506834643910, 15828634621563, 1441534657442,
    807434689163, 1934648674123, 207346839164, 9023466748689, 24036467488329, 7934661034383,
    3673634634657890, 89643466385378, 486803633691932, 8038346664861, 34907334646186, 13903789346662 };
