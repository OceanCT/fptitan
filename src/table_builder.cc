...
void TitanTableBuilder::Add(const Slice& key, const Slice& value) {
  auto debugger = Debugger("|table_builder.cc|TitanTableBuilder.Add:");
  if (!ok()) {
	debugger.dprintln("Stopped due to system already disfunctioning.");
	return;
  }

  ParsedInternalKey ikey;
  status_ = ParseInternalKey(key, &ikey, false /*log_err_key*/);
  if(ikey.type == kTypeValue) debugger.dprintln("Key: %s, Value: %s", ikey.user_key.ToString().c_str(), value.ToString().c_str());
  if (!status_.ok()) {
    return;
  }
  ...
  if (ikey.type == kTypeBlobIndex &&
	...
	debugger.dprintln("run_mode: kFallback, ingest value from blob file");	
    ...
    if (get_status.ok()) {
	  ...
	  AddBlob(ikey, record.value, false);
	...
	if(ikey.type == kTypeValue) {
		debugger.dprintln("Add base directly. Type: value");
	} else if(ikey.type == kTypeBlobIndex) {
		// debugger.dprintln("Add base directly. Type: blob index");
	} 
	AddBase(key, ikey, value);
  }
}
...
void TitanTableBuilder::AddBlob(const ParsedInternalKey& ikey,
  ...
  if(addFP && cf_options_.fpindex_enable) {
	assert(fp_index_ != NULL);
	ctx->fp_enabled = true;
	ctx->fp_features = fp_index_->CalculateFP(record.value.ToString().c_str());
  	Debugger debugger("|table_builder.cc|AddBlob");
	fp_index_->GetBasicInfo();
	debugger.dprintln("CalculateFP from value: %s, Result: %llu, %llu, %llu", record.value.ToString().c_str(), ctx->fp_features[0],
			ctx->fp_features[1], ctx->fp_features[2]);
  }
...
void TitanTableBuilder::AddBlobResultsToBase(
  ...
  for (const std::unique_ptr<BlobFileBuilder::BlobRecordContext>& ctx :
    ...
    if (ctx->has_value) {
	  ...
	  if(ctx->fp_enabled) {
		AddFP(ikey.user_key.ToString(), ctx->fp_features, ctx->new_blob_index);
	  }
...
Status TitanTableBuilder::Finish() {
  ...
  if(status().ok()) {
	for(FPNode fp_node: fpnodes) {
		Debugger debugger("|table_builder.cc|TitanTableBuilder.AddBlobResultsToBase");
		// debugger.dprintln("new blob index, file_number:%d, offset: %d", fp_node.blob_index.file_number, fp_node.blob_index.blob_handle.offset);
		fp_index_->AddRecord(fp_node.key, fp_node.features, fp_node.blob_index);
	}
  }
  return status();
}
...
void TitanTableBuilder::AddFP(std::string key, FPFeatures features, BlobIndex blob_index) {
	fpnodes.push_back({key, features, blob_index});	
} 
