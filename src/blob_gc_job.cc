...
class BlobGCJob::GarbageCollectionWriteCallback : public WriteCallback {
 public:
  ...
  const BlobIndex& blob_index() { return blob_index_; }
...
BlobGCJob::BlobGCJob(BlobGC* blob_gc, DB* db, port::Mutex* mutex,
                     ...
                     std::atomic_bool* shuting_down, TitanStats* stats, FPIndex* fp_index)
    : blob_gc_(blob_gc),
      base_db_(db),
...
Status BlobGCJob::DoRunGC() {
	if(blob_gc_->titan_cf_options().fpindex_enable) {
		...
		// return DoRunGCWithoutFP();
	} else {
		return DoRunGCWithoutFP();
	}
}

typedef struct AddBlobNode {
	std::string key;
	std::string value;
	BlobIndex index;
	bool operator < (AddBlobNode other) const {
...
Status BlobGCJob::DoRunGCWithFP() {
  ...
  std::vector<AddBlobNode> add_blob_nodes;
  ...
  auto addRecordToBlob = [&](BlobRecord record, BlobIndex index) {
	std::unique_ptr<BlobFileBuilder::BlobRecordContext> ctx(
	new BlobFileBuilder::BlobRecordContext);
	InternalKey ikey(record.key, 1, kTypeValue);
  ...
  auto dumpToBlob = [&]() {
	  ...
	  // debugger.dprintln("after sort, vector[0] record, key: %s", add_blob_nodes[0].key.c_str());
	  for(auto i : add_blob_nodes) {
		  // debugger.dprintln("checkBlobFull, add record of key: %s", i.key.c_str());
		  addRecordToBlob({Slice(i.key), Slice(i.value)}, i.index);
	  }
  ...
  for (; gc_iter->Valid(); gc_iter->Next()) {
	...
	for(auto tmp: similar_records) {
		...
		bool record_discardable = false;
		Status record_status;
		record_status = DiscardEntry(similar_record_key, similar_record_index, &record_discardable);
		if(!record_status.ok())	continue;
		if(discardable) continue;
		// above approved
		BlobRecord similar_record;
		PinnableSlice buffer;
		Status get_status;
		debugger.dprintln("Before getting record from storage");  
		mutex_->Lock();
  		auto storage = blob_file_set_->GetBlobStorage(blob_gc_->column_family_handle()->GetID()).lock();
  		mutex_->Unlock();
		get_status = storage->Get(TitanReadOptions(), similar_record_index, &similar_record, &buffer);
		if(!get_status.ok()) {
			debugger.dprintln("Similar Record Key: %s, get status: %s", similar_record_key.c_str(), get_status.ToString().c_str());
			continue;
		}
...
Status BlobGCJob::DoRunGCWithoutFP() {
  Debugger debugger("|blob_gc_job.cc|DoRunGCWithoutFP:");
  debugger.dprintln("DoRunGCWithoutFP");
  Status s;
  ...
  for (; gc_iter->Valid(); gc_iter->Next()) {
    ...
    file_size += blob_index.blob_handle.size;
    BatchWriteNewIndices(contexts, &s);
...
Status BlobGCJob::RewriteValidKeyToLSM() {
  Debugger debugger("|blob_gc_job.cc|RewriteValidKeyToLSM:");
  ...
  for (auto& write_batch : rewrite_batches_) {
	...
	const auto& blob_index = write_batch.second.blob_index();
    if (s.ok()) {
	  ...
	  // with fp, this is necessary
	  debugger.dprintln("drop with fp of {file:%d, size:%d}", blob_index.file_number, new_blob_index.blob_handle.size);	
	  dropped[blob_index.file_number] += new_blob_index.blob_handle.size;
	} else if (s.IsBusy()) {
      metrics_.gc_num_keys_overwritten++;
