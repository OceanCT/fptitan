...
Status BlobFileCache::Get(const ReadOptions& options, uint64_t file_number,
  ...
  Debugger debugger = Debugger("|blob_file_cache.cc|Get"); 
  Cache::Handle* cache_handle = nullptr;
  Status s = FindFile(file_number, file_size, &cache_handle);
  debugger.dprintln("Find file of number: %d, Status: %s", file_number, s.ToString().c_str());
  if (!s.ok()) return s;
  ...
  debugger.dprintln("Find Record of file of number: %d, Status: %s", file_number, s.ToString().c_str());
  return s;
}
